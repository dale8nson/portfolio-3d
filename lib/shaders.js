...
const cubeMapPixelFragShader = /* GLSL */`
...
const pixellateFragShader = /* GLSL */`
...
void main() {
  ...
  x1 = ceil(vUv.x * resX1) / resX1;
  y1 = ceil(vUv.y * resX1) / resX1;
  x2 = ceil(vUv.x * resX2) / resX2;
  y2 = ceil(vUv.y * resX2) / resX2;

  vec3 rgb1 = color;

  rgb1 = texture2D(map1, vec2(x1, y1)).rgb * ceil((1.0 - colorBlend) * steps) / steps;

  vec3 rgb2 = color;

  rgb2 = texture2D(map2, vec2(x2, y2)).rgb * ceil(colorBlend * steps) / steps;
...
export const pixellate = (({ map1, map2, cubeMap1, cubeMap2, color }) => {
  const mat = {
    ...
    vertexShader,
    fragmentShader: cubeMap1 ? cubeMapPixelFragShader : pixellateFragShader,
  }
